...
sensor vote_algo(int obsv_vec[][MAX_DIMENSION], int graph_matrix[][MAX_NUM_SENSOR], int num_sensor, int dimension, int k){
    sensor * sensors = malloc(sizeof(sensor) * num_sensor);
    for(int i = 0; i < num_sensor; i++){
        memcpy(sensors[i].obsv_vec, obsv_vec[i], sizeof(int) * MAX_DIMENSION);
    }
    vote(sensors, graph_matrix, num_sensor, dimension, k * 1589);
...
int main(int argc, char **argv){
    ...
    clock_t t; 
    t = clock();
    sensor res_vec = vote_algo(obsv_vec, graph_matrix, MAX_NUM_SENSOR, dimension, k1);
    t = clock() - t; 
